...
public class aqi extends AbstractEOptimizer {
   private static final StructuredLogger A = aco.pC(aqi.class);
   boolean pC = true;
      ...
      for (BasicBlock var3 : this.cfg) {
         ...
         while (var4 < var3.size()) {
            if (var3.size() >= 2 && ((IEStatement)var3.get(var4)).isNop()) {
               int var5 = akt.pC(this.ectx, var3, var4, true);
               if (var5 != 0) {
                  var1++;
                  if (var5 == 1) {
                     continue;
         ...
         for (int var13 = 1; var13 < this.cfg.size() - 1; var13++) {
            ...
            if (var14.size() == 1 && ((IEStatement)var14.get(0)).isNop()) {
               ...
               if (var9 == var6 + var15.getSize() && (int)var7.getEndAddress() == var6) {
                  ...
                  acj.pC(new RuntimeException("Inconsistency detected"), this.ectx);
               }
            }
         }
