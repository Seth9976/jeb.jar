...
public class atd {
   private static final ILogger pC = GlobalLog.getLogger(atd.class);
   ...
   private static void A() throws Throwable {
      ...
      File var2 = new File(IO.getHomeFolder(), ".jeb/z3");
      if (!var2.isDirectory()) {
         var2.mkdirs();
      }
   ...
   static {
      try {
         A();
         A = true;
         pC.debug("Z3 initialization is done.");
      } catch (Throwable var1) {
         pC.debug("Z3 initialization failed! Some gendec/dexdec optimizers relying on that solver will be disabled.");
         pC.catchingSilent(var1);
      }
   }
}
