package com.pnfsoftware.jeb.corei.debuggers.android.vm;

import com.pnfsoftware.jeb.core.units.code.debug.ITypedValue;
import com.pnfsoftware.jeb.util.logging.GlobalLog;
import com.pnfsoftware.jeb.util.logging.ILogger;
import com.pnfsoftware.jebglobal.rG;
import com.pnfsoftware.jebglobal.wX;
import java.io.IOException;

public class oP extends Kr {
   private static final ILogger pC = GlobalLog.getLogger(oP.class);
   private boolean A = false;
   private rG kS;
   private String wS;
   private Tb UT;
   private ITypedValue E;

   public oP(String var1, int var2, Tb var3, Kr.Av var4, rG var5, String var6) {
      super(var1, null, var2, var3, var4);
      this.kS = var5;
      this.wS = var6;
      this.UT = var3;
   }

   @Override
   public ITypedValue getTypedValue() {
      this.wS();
      return this.E;
   }

   @Override
   public boolean setTypedValue(ITypedValue var1) {
      this.wS();
      boolean var2 = super.setTypedValue(var1);
      if (var2) {
         this.E = super.getTypedValue();
      }

      return var2;
   }

   private void wS() {
      if (!this.A) {
         try {
            this.E = this.UT.pC(this.kS, this.wS);
            this.A = true;
         } catch (wX | IOException var2) {
            pC.catching(var2);
         }
      }
   }

   @Override
   public rG pC() {
      return this.kS;
   }

   public static Kr pC(String var0, int var1, Tb var2, Kr.Av var3, rG var4, String var5) throws wX, IOException {
      switch (var4.pC) {
         case 66:
         case 67:
         case 68:
         case 70:
         case 73:
         case 74:
         case 83:
         case 86:
         case 90:
            return new Kr(var0, var2.pC(var4, var5), var1, var2, var3);
         case 69:
         case 71:
         case 72:
         case 75:
         case 76:
         case 77:
         case 78:
         case 79:
         case 80:
         case 81:
         case 82:
         case 84:
         case 85:
         case 87:
         case 88:
         case 89:
         case 91:
         case 92:
         case 93:
         case 94:
         case 95:
         case 96:
         case 97:
         case 98:
         case 99:
         case 100:
         case 101:
         case 102:
         case 103:
         case 104:
         case 105:
         case 106:
         case 107:
         case 108:
         case 109:
         case 110:
         case 111:
         case 112:
         case 113:
         case 114:
         case 115:
         case 116:
         default:
            return new oP(var0, var1, var2, var3, var4, var5);
      }
   }
}
