...
import com.pnfsoftware.jebglobal.cdo;
import com.pnfsoftware.jebglobal.cdp;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

@SerDisabled
public class KD extends AbstractTreeDocument {
   private final IELFUnit pC;
   private final INativeCodeUnit A;
   private List kS;

   public KD(IELFUnit var1) {
      this.pC = var1;
      this.A = (INativeCodeUnit)UnitUtil.findFirstChildByType(var1, INativeCodeUnit.class, false);
      List var2 = var1.getDwarfDIEs();
      if (var2 != null && !var2.isEmpty()) {
         this.kS = new ArrayList();

         for (IDIE var4 : var2) {
            this.kS.add(new cdp(var4));
         }
      }
   }
   ...
   public String coordinatesToAddress(INodeCoordinates var1) {
      return this.coordinatesToAddress(var1, var1x -> {
         ...
         if (var2 instanceof cdp) {
            return pC(((cdp)var2).pC());
         } else {
            return var2 instanceof cdo ? pC(((cdp)var1x.get(var1x.size() - 2)).pC()) + "+" + var1.getPath().get(var1.getPath().size() - 1) : null;
         }
      });
   }

   private static String pC(IDIE var0) {
   ...
   public INodeCoordinates addressToCoordinates(String var1) {
      if (var1 != null && !var1.startsWith("sub_")) {
         ...
         for (int var8 = 0; var8 < this.kS.size(); var8++) {
            if (((cdp)this.kS.get(var8)).pC().getOffset() == var5) {
               var7 = ((cdp)this.kS.get(var8)).pC();
               var4 = var8;
               break;
            }
         }

         for (int var12 = 0; var12 < this.kS.size(); var12++) {
   ...
   public String getReferencedAddress(INode var1) {
      if (var1 instanceof cdo) {
         IDIEAttribute var2 = ((cdo)var1).pC();
         ...
         if (var2.getType() == Dwarf.DwarfFormType.address) {
            long var3 = this.A.getVirtualImageBase() - this.pC.getLoaderInformation().getImageBase() + (Long)var2.getForm();
