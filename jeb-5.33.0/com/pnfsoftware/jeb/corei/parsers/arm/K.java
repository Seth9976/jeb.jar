...
import com.pnfsoftware.jeb.corei.parsers.arm.converter.GK;
import com.pnfsoftware.jeb.corei.parsers.arm.converter.HE;
import com.pnfsoftware.jeb.corei.parsers.arm.converter.gJ;
import com.pnfsoftware.jeb.corei.parsers.arm.converter.p;
import com.pnfsoftware.jeb.corei.parsers.arm.converter.vi;
import com.pnfsoftware.jeb.util.collect.Sets;
import com.pnfsoftware.jeb.util.format.Strings;

public class K extends AbstractNativeDecompilerExtension {
   @Override
   ...
   public ChainedOperationResult customizeIntermediateOptimizer(INativeDecompilerContext var1, IEMasterOptimizer var2) {
      if (var1.getNativeContext().getProcessor().getType().is64Bit()) {
         var2.registerOptimizer(new gJ());
      } else {
         var2.registerOptimizer(new vi());
         var2.registerOptimizer(new p());
         var2.registerOptimizer(new GK());
      }

      var2.addDisregardedOutputBits(Sets.createNonNulls(543, 540, 542, 541, 528, 536, 512, 539, 553, 552, 551, 550, 517));
   ...
   public ChainedOperationResult augmentSimulationContext(
      ...
      if (var6 instanceof IEUntranslatedInstruction && Strings.equalsIgnoreCase(((IEUntranslatedInstruction)var6).getNativeMnemonic(), "svc")) {
         ...
         if (var11 != 16 && var11 != 32) {
            ...
            var10 = var7.getValueAsLongSafe(HE.A(8));
         } else {
            var9 = ProcessorType.ARM;
            var10 = var7.getValueAsLongSafe(HE.pC(7));
         }
