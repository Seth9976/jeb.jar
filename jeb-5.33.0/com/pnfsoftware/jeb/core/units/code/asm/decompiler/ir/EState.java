...
@SerDisabled
public class EState {
   private static final StructuredLogger logger = aco.pC(EState.class);
   ...
   public EState(IEGlobalContext var1) {
      this.gctx = var1;
      this.vm = new awp(var1.getAddressBitsize());
   ...
   public void setValue(IEGeneric var1, IEImm var2) {
      if (var2 == null) {
         throw new IllegalArgumentException();
      } else if (var1.getBitsize() != var2.getBitsize()) {
         throw new IllegalArgumentException("Unexpected bitsize for value");
      } else {
         if (var1 instanceof IEVar var5) {
            this.setValue(var5, var2);
         } else {
            if (!(var1 instanceof IESlice var3) || !(var3.getWholeExpression() instanceof IEVar var4)) {
               throw new IllegalArgumentException();
            }

            IEImm var10 = this.getValueInternal(var4.getId());
            if (var10 == null) {
               throw new IllegalArgumentException();
            }

            if (var3.getBitStart() > 0 && var3.getBitEnd() < var10.getBitsize()) {
               IEGeneric var12 = var10.slice(0, var3.getBitStart());
               IEGeneric var14 = var10.slice(var3.getBitEnd());
               IEImm var9 = this.gctx.createCompose(var12, var2, var14).evaluate(null);
               this.setValue(var4, var9);
            } else if (var3.getBitStart() == 0 && var3.getBitEnd() < var10.getBitsize()) {
               IEGeneric var11 = var10.slice(var3.getBitEnd());
               IEImm var13 = this.gctx.createCompose(var2, var11).evaluate(null);
               this.setValue(var4, var13);
            } else {
               if (var3.getBitStart() <= 0 || var3.getBitEnd() != var10.getBitsize()) {
                  throw new IllegalArgumentException();
               }

               IEGeneric var7 = var10.slice(0, var3.getBitStart());
               IEImm var8 = this.gctx.createCompose(var7, var2).evaluate(null);
               this.setValue(var4, var8);
            }
         }
      }
   }
   ...
   public synchronized boolean writeMemoryByte(long var1, byte var3) {
      this.tmp1[0] = var3;
      return this.writeMemory(var1, this.tmp1);
   }
