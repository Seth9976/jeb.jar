...
public class GenericCodeFormatter {
   ...
   private void init() {
      this.propcache = CacheBuilder.newBuilder().expireAfterWrite(2L, TimeUnit.SECONDS).build();
   }
   ...
   protected void formatMnemonic(long var1, IInstruction var3, boolean var4, CodeDocumentPart var5) {
      ...
      var6 += var7.length();
      if (this.getCountOfFormattedOperands(var3) > 0) {
   ...
   protected int getCountOfFormattedOperands(IInstruction var1) {
      return var1.getOperands().length;
   }
   ...
   private boolean formatOperandGeneric(long var1, IInstruction var3, IInstructionOperandGeneric var4, int var5, int var6, CodeDocumentPart var7) {
      ...
      switch (var8) {
         ...
         case 9:
            ...
            if (var16 == null) {
               if (!this.isDoNotAttemptImmediateToAddressResolution() && !Booleans.isFalse(yj.A(this.pbcu.getCodeAnalyzer(), var9))) {
                  String var17 = this.pbcu.pC(var9, AutoLabelPolicy.ITEM);
                  if (var17 != null) {
                     var7.append("offset ");
   ...
   public final void formatImm(long var1, IInstructionOperandSized var3, long var4, int var6, CodeDocumentPart var7) {
      ...
      if (var8 == null) {
         if (!this.isDoNotAttemptImmediateToAddressResolution()) {
            String var9 = this.pbcu.pC(var4, AutoLabelPolicy.ITEM);
            if (var9 != null) {
               var7.append("offset ");
   ...
   public NumberFormatter getNumberFormatter(INativeDataItem var1) {
      NumberFormatter var2 = ((aum)var1).pC(false);
      if (var2 == null) {
   ...
   protected boolean getCachedBooleanProperty(String var1) {
      try {
         return (Boolean)this.propcache.get(var1, () -> this.getCodeUnit().getPropertyManager().getBoolean(var1));
      } catch (ExecutionException var2) {
         return false;
      }
   }

   protected int getCachedIntegerProperty(String var1) {
      try {
         return (Integer)this.propcache.get(var1, () -> this.getCodeUnit().getPropertyManager().getInteger(var1));
      } catch (ExecutionException var2) {
         return 0;
      }
   }

   protected String getCachedStringProperty(String var1) {
      try {
         return (String)this.propcache.get(var1, () -> this.getCodeUnit().getPropertyManager().getString(var1));
      } catch (ExecutionException var2) {
         return "";
      }
   }
