...
import com.pnfsoftware.jebglobal.NW;
import com.pnfsoftware.jebglobal.Rk;
import com.pnfsoftware.jebglobal.Ws;
import com.pnfsoftware.jebglobal.ckx;
import com.pnfsoftware.jebglobal.hi;
import com.pnfsoftware.jebglobal.vJ;
import java.io.File;
...
public abstract class AbstractClientContext extends AbstractContext implements IClientContext {
   ...
   private static final String UPDATE_FILENAME = ckx.pC(new byte[]{54, 31, 20, 24, 6, 12, 73, 18, 29, 80}, 2, 183);
   private static final String UPDATE_PASS_FILENAME = ckx.pC(new byte[]{86, 5, 20, 5, 21, 17, 75, 94, 7, 19}, 1, 35);
   ...
   public void start() throws JebException {
      logger.info(
         ...
         "JEB",
         app_ver.toString(),
         ckx.pC(new byte[]{16, 26, 0, 28, 0, 68, 37, 4, 21, 67, 67, 67, 108, 68, 88, 77, 80, 92, 66, 0, 80, 73, 18, 118, 116, 118}, 2, 176)
      );
      ...
      if (fromString == null || fromString.compareTo(app_ver) < 0) {
         ...
         this.onUpdatedSoftware(null, fromString);
      }

      this.pm.setString(".LastVersionRun", app_ver.toString());
      boolean b = this instanceof Ws;
   ...
   private void prepareCheckLicenseKey() {
      ...
      if (!Strings.isBlank(string)) {
         ...
         for (int i = 0; i < length; i++) {
            if (new NW(ks[i]).pC(string, array)) {
               b = true;
               break;
            }
         }
      ...
      if (!b) {
         Rk.A();
         NW nw = new NW(vJ.A());
         String retrieveLicenseKey = this.retrieveLicenseKey(nw.pC());
         if (!nw.pC(retrieveLicenseKey, array)) {
   ...
   public static final String generateLicenseInformation() {
      ...
      if (!Licensing.isCommonBuild()) {
         Strings.ff(sb, "\n");
         Strings.ff(sb, S.L("Licensed to %s"), Strings.ff("%s (%s)", ckx.pC(new byte[]{0, 55, 38}, 2, 176), ckx.pC(new byte[]{19, 33, 54}, 2, 176)));
   ...
   private InetSocketAddress getControllerAddress() {
