...
public class chz extends AbstractDOptimizer {
   @Override
   public int perform() {
      ...
      for (BasicBlock var4 : this.cfg) {
         ...
         while (var5 < var4.size()) {
            if (var4.size() >= 2 && ((IDInstruction)var4.get(var5)).getOpcode() == DOpcodeType.IR_NOP) {
               var2++;
      ...
      while (var12 < this.cfg.size()) {
         ...
         if (var13.size() != 1) {
            ...
            IDInstruction var14 = (IDInstruction)var13.getLast();
            if (var14.getOpcode() != DOpcodeType.IR_NOP) {
               var12++;
            } else {
