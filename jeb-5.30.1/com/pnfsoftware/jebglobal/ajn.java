...
public class ajn extends AbstractCBlockOptimizer {
   private static final StructuredLogger q = aeg.q(ajn.class);
   private boolean RF;
   ...
   protected int optimizeBlock(ICBlock var1) {
      ...
      for (int var3 = 0; var3 < var1.size(); var3++) {
         ...
         ICExpression var5 = this.q(var4);
         if (var5 != null) {
            AtomicBoolean var6 = new AtomicBoolean(false);
            AtomicInteger var7 = new AtomicInteger(0);
            boolean var8 = this.q(var5, var1, var3 + 1, null, var6, var7) && this.q(var5);
            if (var8 && !var6.get() && var1 != this.m.getBody()) {
               var8 = this.q(var5, this.m.getBody(), 0, var1, var6, var7);
               var6.set(true);
            }

            if (var8 && (var6.get() || var1 == this.m.getBody()) && this.q(var1, var3)) {
               var3--;
               var2++;
            }
   ...
   private boolean q(ICExpression var1, ICBlock var2, int var3, ICBlock var4, AtomicBoolean var5, AtomicInteger var6) {
      ...
      for (int var8 = var3; var8 < var2.size(); var8++) {
         ...
         if (!this.q(var9, var10, var11, var5, var4)) {
            return false;
         }
         ...
         for (ICLeftExpression var13 : var10) {
            if (!Boolean.FALSE.equals(ajk.q(var13, var1, true))) {
               return false;
            }
         }
         ...
         if (var7 != null && var7 instanceof ICIdentifier) {
            for (ICLeftExpression var16 : var11) {
               if (!Boolean.FALSE.equals(ajk.q(var16, var7, true))) {
                  var7 = null;
               }
            }
         }
   ...
   private boolean q(ICStatement var1, List var2, List var3, AtomicBoolean var4, ICBlock var5) {
      if (var1 == var5) {
         return true;
      } else {
         ArrayList var6 = new ArrayList();
         if (var1 instanceof ICAssignment) {
            if (!ajk.q(var1, var2, var6)) {
               return false;
            } else {
               ICLeftExpression var14 = ajk.q(var1);
               if (var14 == null) {
                  return false;
               } else {
                  var3.add(var14);
            ...
            var4.set(true);
            return ajk.q(var1, var2, var6);
         } else if (var1 instanceof ICBlock) {
            for (ICStatement var15 : (ICBlock)var1) {
               boolean var16 = this.q(var15, var2, var3, var4, var5);
               if (!var16) {
                  return var16;
               }
            }
            ...
            for (int var11 = 0; var11 < var10.size(); var11++) {
               if (var8) {
                  if ((var11 < var10.size() - 1 || !var7.hasDefaultBlock()) && !ajk.q(((ICIfStm)var7).getBranchPredicate(var11), var2, var6)) {
                     return false;
                  }
               } else if (var11 == 0) {
                  ...
                  if (!ajk.q(((ICSwitchStm)var7).getSwitchedExpression(), var2, var6)) {
                     return false;
                  }
               }

               AtomicBoolean var12 = new AtomicBoolean(false);
               if (!this.q((ICStatement)var10.get(var11), var2, var3, var12, var5)) {
                  return false;
               }
   ...
   private boolean q(ICExpression var1) {
      if (var1 instanceof ICIdentifier var2) {
         ...
         if (this.RF && var2.getIdentifierClass() == CIdentifierClass.LOCAL) {
            return true;
         }
      }
   ...
   private ICExpression q(ICElement var1) {
      if (var1 instanceof ICAssignment) {
