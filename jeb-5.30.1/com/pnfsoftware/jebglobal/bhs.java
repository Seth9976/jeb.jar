package com.pnfsoftware.jebglobal;

import com.pnfsoftware.jeb.core.units.code.android.DexUtil;
import com.pnfsoftware.jeb.core.units.code.android.dex.IDexAnnotation;
import com.pnfsoftware.jeb.util.collect.ArrayUtil;
import com.pnfsoftware.jeb.util.serialization.annotations.Ser;
import com.pnfsoftware.jeb.util.serialization.annotations.SerId;
import java.util.List;

@Ser
public final class bhs implements IDexAnnotation {
   @SerId(1)
   private int q;
   @SerId(2)
   private bht[] RF;

   private bhs(int var1, int var2) {
      this.q = var1;
      this.RF = new bht[var2];
   }

   @Override
   public int getTypeIndex() {
      return this.q;
   }

   @Override
   public List getElements() {
      return ArrayUtil.asView(this.RF);
   }

   static bhs q(bjw var0, byte[] var1, int var2, int[] var3) {
      int[] var4 = new int[1];
      int var6 = DexUtil.bytearrayULEB128ToInt(var1, var2, var4);
      var6 = var0.RF(var6);
      int var5 = var2 + var4[0];
      int var7 = DexUtil.bytearrayULEB128ToInt(var1, var5, var4);
      var5 += var4[0];
      bhs var8 = new bhs(var6, var7);

      for (int var9 = 0; var9 < var7; var9++) {
         int var10 = DexUtil.bytearrayULEB128ToInt(var1, var5, var4);
         var10 = var0.q(var10);
         var5 += var4[0];
         bia var11 = bia.q(var0, var1, var5, var4);
         var5 += var4[0];
         bht var12 = new bht(var10, var11);
         var8.RF[var9] = var12;
      }

      var3[0] = var5 - var2;
      return var8;
   }
}
