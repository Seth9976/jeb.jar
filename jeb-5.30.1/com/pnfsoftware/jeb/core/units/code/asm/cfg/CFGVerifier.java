...
public class CFGVerifier {
   ...
   private void verifyBranchingInstruction(BasicBlock var1, IInstruction var2, IFlowInformation var3) throws CfgVerificationException {
      ...
      if (!var7.isEmpty()) {
         ...
         List var11 = var1.getOutputOffsets();
         HashSet var12 = new HashSet(var11);
         if (var11.size() != var12.size()) {
            throw new CfgVerificationException("Block %X: Duplicate edges: %s", var1.getBase(), Longs.formatHexCollection(var11));
         } else {
            boolean var10 = false;
            if (var5.size() != var1.outsize()) {
               if (this instanceof all
                  && ((all)this).q().getRoutine().getData().getTrampolineTarget() != null
                  && this.cfg.size() == 1
                  && var5.size() == 1
                  && var1.outsize() == 0) {
                  var10 = true;
               ...
               if (!var10) {
                  throw new CfgVerificationException(
                     "Block %X: Number of targets (%d) differ from number of edges (%d)", var1.getBase(), var5.size(), var1.outsize()
                  );
               }
            }

            if (!var10 && !var6.equals(var12)) {
               throw new CfgVerificationException(
                  "Block %X: Targets are not synchronized with edges: targets=%s / edges=%s",
                  var1.getBase(),
                  Longs.formatHexCollection(var6),
                  Longs.formatHexCollection(var12)
               );
            } else {
   ...
   private void verifyFallthrough(BasicBlock var1, List var2) throws CfgVerificationException {
      ...
      for (BasicBlock var7 : var1.getOutputBlocks()) {
