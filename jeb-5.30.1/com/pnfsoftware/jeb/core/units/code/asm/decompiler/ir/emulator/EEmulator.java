...
public class EEmulator {
   ...
   public EEmulator(EState var1) {
      if (var1 != null && var1.getGlobalContext() != null) {
         ...
         if (this.proctype == ProcessorType.ARM64) {
            var1.setPointerSanitizer(new EEmulator$1(this));
         }
      } else {
   ...
   public void addArgument(long var1, INativeType var3) {
      this.args.add(alu.q(var1, var3.getSize() * 8));
   }

   public void addArgument(byte[] var1, INativeType var2) {
      this.args.add(alu.q(var1, var2.getSize() * 8));
   }
      ...
      if (this.isExecRoutine()) {
         ...
         if (var1 == null) {
            ...
            try {
               ...
               this.writeStorage(var14, alu.q(this.retAddr, this.addrsize * 8));
               this.args.clear();
            } finally {
   ...
   private void runFull() {
      ...
      if (this.pseudoIRContext == null) {
         this.pseudoIRContext = new aml((alr)this.gctx);
      ...
      while (true) {
         EEmulator.ConvertedInsnCachedIREntry var11;
         EEmulator.RegisteredRoutine var13;
         IInstruction var21;
         while (true) {
            ...
            if (var1) {
               ...
               int var22 = this.state.getEvaluationCount();
               log("Emulation COMPLETED - routine returned (%d iter. - total in State: %d)", var22 - var8, var22);
               return;
            }

            var11 = (EEmulator.ConvertedInsnCachedIREntry)this.mapConverterCachedEntries.get(var4);
            if (var11 == null) {
               try {
                  var21 = var3.parseAt(var2, var4);
               } catch (ProcessorException var18) {
                  var21 = null;
               }

               var11 = new EEmulator.ConvertedInsnCachedIREntry(var4, var21, null);
               this.mapConverterCachedEntries.put(var4, var11);
            } else {
               var21 = var11.nativeInsn;
            }

            Boolean var12 = this.hooksEvaluateAt(var4, var11.nativeInsn);
            if (var12 == null) {
               if (var11.nativeInsn == null || !this.hooksEvaluateSyscall(var4, var11.nativeInsn)) {
                  var13 = (EEmulator.RegisteredRoutine)this.mapRegisteredRoutines.get(var4);
                  if (var13 == null) {
                     break;
                  }

                  var12 = this.hooksEvaluateExternal(var13.name, var13.routine);
                  if (var12 == null) {
                     break;
                  }

                  if (!var12) {
                     throw new EvaluationException(Strings.ff("evaluateExternal() failed at: 0x%X: %s", var4, var13.name));
                  }
               }
            } else if (!var12) {
               throw new EvaluationException(Strings.ff("evaluateAt() failed at: 0x%X: %s", var4, var11.nativeInsn));
            }
         }

         if (var21 == null) {
            if (var13 != null) {
               throw new EvaluationException(Strings.ff("Cannot eval routine: %s", var13));
            }

            throw new EvaluationException(Strings.ff("Cannot parse at address: 0x%X", var4));
         }

         if (var11.stmlist == null) {
            List var10 = var6.r;
            var6.address = var4;
            var6.insn = var21;
            var6.irAddress = 0;
            ...
            EUtil.setLowerLevelAddress(var4, var10);
            var11.stmlist = new ArrayList(var10);
         }

         List var23 = var11.stmlist;
         int var14 = 0;
         this.state.setVirtualPC(var14);
         boolean var15 = false;
         if (var23.size() == 1 && ((IEStatement)var23.get(0)).isUntranslatedInstruction()) {
            IEUntranslatedInstruction var16 = ((IEStatement)var23.get(0)).asUntranslated();
            Boolean var25 = this.hooksEvaluateUntranslated(var16, var21);
            if (var25 == null || !var25) {
               throw new EvaluationException(Strings.ff("evaluateUntranslated() could not do the job at: 0x%X: %s = %s", var4, var21, var16));
            }
         ...
         if (!var15) {
            for (; var14 >= 0 && var14 < var23.size(); var14 = this.state.getVirtualPC()) {
               IEStatement var26 = (IEStatement)var23.get(var14);

               try {
                  this.setPCAddress(var4);
                  this.lastEvalResult = var26.evaluate(this.state);
               } catch (Exception var19) {
                  log("FAILED TO EVALUATE: %s  { from native insn: 0x%X: %s }", var26, var4, var21);
                  throw var19;
               }
            }
         }
         ...
         if (this.state.getValueSafe(this.idPC) == null) {
            this.setPCAddress(var4 + var21.getSize());
         }
      }
   }
   ...
   public IEImm readStorage(StorageEntry var1) {
      switch (var1.getType()) {
         case STACK:
            ...
            try {
               if (this.slotsize == 4) {
                  return alu.q(this.state.getMemory().readInt(var8), this.slotsize * 8);
               }

               if (this.slotsize == 8) {
                  return alu.q(this.state.getMemory().readLong(var8), this.slotsize * 8);
               }
            } catch (Exception var6) {
            }
      ...
      switch (var1.getType()) {
         case STACK:
            if (var1.getValueAsStackIndex() != 0) {
               ...
               this.state.setValue(this.SP, var9._add(alu.q((1 + var2) * this.slotsize, var9.getBitsize())));
         ...
         case REGISTER:
            ...
            if (var2 != 0) {
               ...
               this.state.setValue(this.SP, var7._add(alu.q(var2 * this.slotsize, var7.getBitsize())));
            }
   ...
   private class MemHooks implements IEStateHooks {
      ...
      public Integer onReadMemory(EState var1, long var2, byte[] var4) {
         ...
         if (var5 != null && (var5.getMemorySize() == var4.length || var4.length == EEmulator.this.addrsize) && var5 instanceof axu) {
            INativeMethodItem var6 = ((axu)var5).cC();
            if (var6 != null && ((axp)var6).IN() == null && var6.getMemoryAddress() == null) {
