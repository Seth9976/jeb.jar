...
public class EExpressionMatcher {
   ...
   public EExpressionMatcher(INode var1, Map var2) {
      if (var1 == null) {
         ...
         this.template = var1;
         if (var1 instanceof Node && ((Node)var1).descMaxDepth < 0) {
            Util.setMaxDepths(var1);
         }
   ...
   private Boolean checkCommutative(List var1, Node var2, O var3, int var4) {
      if (commuteForkMode > 0 && var3.isCommutative()) {
         if (var2.opnds[0] instanceof Leaf && var2.opnds[1] instanceof Leaf) {
            boolean var12 = false;
            Leaf var13 = (Leaf)var2.opnds[0];
            Leaf var14 = (Leaf)var2.opnds[1];
            int var8 = var13.id;
            int var9 = var14.id;
            IEGeneric var10 = (IEGeneric)this.map.get(var8);
            IEGeneric var11 = (IEGeneric)this.map.get(var9);
         ...
         if (commuteForkMode >= 2 && this.canFork()) {
            EExpressionMatcher var5 = this.fork(this);
            boolean var7 = false;
            boolean var6 = var5.isEqual((IEGeneric)var1.get(0), var2.opnds[0], var4 + 1);
            this.forkcount = var5.forkcount;
            if (var6) {
               var7 = var5.isEqual((IEGeneric)var1.get(1), var2.opnds[1], var4 + 1);
               this.forkcount = var5.forkcount;
            }

            if (!var7) {
               var5 = this.fork(this);
               var6 = var5.isEqual((IEGeneric)var1.get(0), var2.opnds[1], var4 + 1);
               this.forkcount = var5.forkcount;
               if (var6) {
                  var7 = var5.isEqual((IEGeneric)var1.get(1), var2.opnds[0], var4 + 1);
                  this.forkcount = var5.forkcount;
               }
            }

            if (var6 && var7) {
               this.map.putAll(var5.map);
               return true;
            }
